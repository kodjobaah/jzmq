...
public class FacebookPostTask extends AsyncTask<Void, Void, Boolean> {
	...
	public FacebookPostTask(String url, WhatAmIdoing context, Session facebookSession) {
		this.url = url;
	...
	protected void onPostExecute(final Boolean success) {
		...
		if (success) {
			...
			params.putString("description", context.getString(R.string.facebook_feed_dialog_description));
			params.putString("link",
					url);
		    params.putString("name", context.getString(R.string.facebook_feed_dialog_name));
		    params.putString("picture", context.getString(R.string.facebook_feed_dialog_picture));
		    params.putString("display", context.getString(R.string.facebook_feed_dialong_display));

		    requestsDialog = (new WebDialog.FeedDialogBuilder(
					context, facebookSession, params)).setOnCompleteListener(
						...
						public void onComplete(Bundle values,
							...
							if (error != null) {
								if (error instanceof FacebookOperationCanceledException) {
									Log.i(TAG,"request cancelled["+ error.getMessage() + "]");
									UtilsWhatAmIdoing.displayNetworkProblemsForInvitesDialog(context);
								} else {
									UtilsWhatAmIdoing.displayNetworkProblemsForInvitesDialog(context);
									Log.i(TAG,"network error["+ error.getMessage() + "]");

								}
							} else if (postId != null) {
								Log.i(TAG, "Request Sent");
								UtilsWhatAmIdoing.displaySuccessInvitesFacebbokDialog(context);
								CloseFacebookFeedDialogTask cffdt = new CloseFacebookFeedDialogTask(facebookPostTask);
